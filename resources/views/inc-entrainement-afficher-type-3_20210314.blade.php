...
foreach ($enregistrements as $enregistrement) {
	...
	<div class="ml-3">
	<?php
	$sujet2 = ($enregistrement->sujet_id == $enregistrement->sujet_id1) ? $enregistrement->sujet_id2 : $enregistrement->sujet_id1;
	$sujets = App\Sujet::whereIn('id',[$enregistrement->sujet_id, $sujet2])->orderByRaw("field(id,".implode(',',[$enregistrement->sujet_id, $sujet2]).")")->get();
	...
	foreach ($sujets as $sujet) {
		...
		if ($sujet->id == $enregistrement->sujet_id){
			?>
			<div class="card border-success mb-1"><div class="card-body p-3"><?php echo $sujet_texte ?></div></div>
			<div class="text-monospace small text-success mb-2" style="color:silver">sujet tiré au sort</div>
			<?php
		} else {
			?>
