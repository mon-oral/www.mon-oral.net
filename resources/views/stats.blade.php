			...
			<div class="container">
				...
				<div>
					...
					*/
					...
					// INSCRIPTIONS			
					$inscriptions_weeks = [];
					App\User::selectRaw('DATE_FORMAT(created_at,"%Y-%v") AS week_number, COUNT(id) AS id_count')
						->groupBy("week_number")
						->get()
						->map(function($item) use(&$inscriptions_weeks) {
							$inscriptions_weeks[$item->week_number] = $item->id_count;
						});
					$inscriptions_data = [];
					foreach ($period_week as $dt) {
						if (!empty($inscriptions_weeks[$dt->format("Y-W")])) {
							$inscriptions_data[] = '{t:"' . $dt->format("Y-W") . '",y:' . $inscriptions_weeks[$dt->format("Y-W")] . '}';
						} else {
					...
					// ENTRAINEMENTS
					$entrainements_weeks = [];
					App\Entrainement::selectRaw('DATE_FORMAT(created_at,"%Y-%v") AS week_number, COUNT(id) AS id_count')
						->groupBy("week_number")
						->get()
						->map(function($item) use(&$entrainements_weeks) {
							$entrainements_weeks[$item->week_number] = $item->id_count;
						});
					$entrainements_data = [];
					foreach ($period_week as $dt) {
						if (!empty($entrainements_weeks[$dt->format("Y-W")])) {
							$entrainements_data[] = '{t:"' . $dt->format("Y-W") . '",y:' . $entrainements_weeks[$dt->format("Y-W")] . '}';
						} else {
					...
					// ACTIVITES
					$activites_weeks = [];
					App\Activite::selectRaw('DATE_FORMAT(created_at,"%Y-%v") AS week_number, COUNT(id) AS id_count')
						->groupBy("week_number")
						->get()
						->map(function($item) use(&$activites_weeks) {
							$activites_weeks[$item->week_number] = $item->id_count;
						});
					$activites_data = [];
					foreach ($period_week as $dt) {
						if (!empty($activites_weeks[$dt->format("Y-W")])) {
							$activites_data[] = '{t:"' . $dt->format("Y-W") . '",y:' . $activites_weeks[$dt->format("Y-W")] . '}';
						} else {
					...
					$entrainements_enregistrements_weeks = [];
					App\Log::selectRaw('DATE_FORMAT(created_at,"%Y-%v") AS week_number, COUNT(id) AS id_count')
						->groupBy("week_number")
						->get()
						->map(function($item) use(&$entrainements_enregistrements_weeks) {
							$entrainements_enregistrements_weeks[$item->week_number] = $item->id_count;
						});
					$entrainements_enregistrements_data = [];
					foreach ($period_week as $dt) {
						if (!empty($entrainements_enregistrements_weeks[$dt->format("Y-W")])) {
							$entrainements_enregistrements_data[] = '{t:"' . $dt->format("Y-W") . '",y:' . $entrainements_enregistrements_weeks[$dt->format("Y-W")] . '}';
						} else {
					...
					$activites_enregistrements_weeks = [];
					App\Activites_enregistrement::selectRaw('DATE_FORMAT(created_at,"%Y-%v") AS week_number, COUNT(id) AS id_count')
						->groupBy("week_number")
						->get()
						->map(function($item) use(&$activites_enregistrements_weeks) {
							$activites_enregistrements_weeks[$item->week_number] = $item->id_count;
						});
					$activites_enregistrements_data = [];
					foreach ($period_week as $dt) {
						if (!empty($activites_enregistrements_weeks[$dt->format("Y-W")])) {
							$activites_enregistrements_data[] = '{t:"' . $dt->format("Y-W") . '",y:' . $activites_enregistrements_weeks[$dt->format("Y-W")] . '}';
						} else {
					...
					$commentaires_enregistrements_weeks = [];
					App\Commentaire::selectRaw('DATE_FORMAT(created_at,"%Y-%v") AS week_number, COUNT(id) AS id_count')
						->groupBy("week_number")
						->get()
						->map(function($item) use(&$commentaires_enregistrements_weeks) {
							$commentaires_enregistrements_weeks[$item->week_number] = $item->id_count;
						});
					$commentaires_enregistrements_data = [];
					foreach ($period_week as $dt) {
						if (!empty($commentaires_enregistrements_weeks[$dt->format("Y-W")])) {
							$commentaires_enregistrements_data[] = '{t:"' . $dt->format("Y-W") . '",y:' . $commentaires_enregistrements_weeks[$dt->format("Y-W")] . '}';
						} else {
					...
					$capsules_enregistrements_weeks = [];
					App\Logs_capsule::selectRaw('DATE_FORMAT(created_at,"%Y-%v") AS week_number, COUNT(id) AS id_count')
						->groupBy("week_number")
						->get()
						->map(function($item) use(&$capsules_enregistrements_weeks) {
							$capsules_enregistrements_weeks[$item->week_number] = $item->id_count;
						});
					$capsules_enregistrements_data = [];
					foreach ($period_week as $dt) {
						if (!empty($capsules_enregistrements_weeks[$dt->format("Y-W")])) {
							$capsules_enregistrements_data[] = '{t:"' . $dt->format("Y-W") . '",y:' . $capsules_enregistrements_weeks[$dt->format("Y-W")] . '}';
						} else {
