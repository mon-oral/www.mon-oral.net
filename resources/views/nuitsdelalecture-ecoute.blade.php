					...
					*/
					...
					foreach($siecles as $siecle) {
						if (in_array($siecle, array_column($liste_lectures, 'siecle'))) {
							echo '<div class="text-light pt-4">' . substr($siecle, 0, -1) . '<sup>e</sup></div>';
						}
						foreach($liste_lectures as $lecteur_audio) {
							if($siecle == $lecteur_audio['siecle'] AND $lecteur_audio['autorisation'] == 0){
								echo '<div class="pb-1" style="color:black">' . $lecteur_audio['auteur'] . '<span style="color:silver"> | </span>' . $lecteur_audio['extrait'] . '</div>';
								echo '<audio controls class="mb-3" style="width:100%;"><source src="/ndll-lfit/lecteur/'. Crypt::encryptString($lecteur_audio['code_audio']) .'" type="audio/mpeg"></audio>';
							}
						}
					}
					?>
